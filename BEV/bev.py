...
def BEV_transform(rot, B, S, H, W, meter_per_pixel, Camera_height):
    ...
    ii, jj = torch.meshgrid(torch.arange(0, S, dtype=torch.float32, device=rot.device),
                            torch.arange(0, S, dtype=torch.float32, device=rot.device))
    ...
    # Normalize theta to be within the range [0, 2��)
...
def main():
    ...
    input_folder = ':data/CVACT'
    output_folder = 'data/CVACT//BEV'
    ...
    Camera_height = -1.5  # Camera height parameter for BEV transformation
